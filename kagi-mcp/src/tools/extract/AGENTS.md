...
## OVERVIEW
MCP `extract` tool: validates URLs, optionally splits per-URL, calls Kagi Extract API, applies per-domain fallback rules, renders Markdown.
...
- Fallbacks: if API returns empty content, substitute configured message
- `--fallback-always` skips API entirely for matched domains

## ANTI-PATTERNS
- Never use `tokio::spawn` in handler — synchronous execution only
